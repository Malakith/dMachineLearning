...
import convNetwork_highlearningrate as model
import numpy as np
...
def train(images, labels, learning_rate, keep_prop, batch_size, epochs, cont = 0):
    with tf.Graph().as_default():
        ...
        images_ph, labels_ph, learning_rate_ph, keep_prop_ph, init_op, train_op, pred_op, eval_op, saver = \
            model.buildModel(images, labels, keep_prop, batch_size, epochs, learning_rate, threads=8)
        ...
        with tf.Session() as session:
            ...
            try:
                while not coord.should_stop():
                    _, loss_value = session.run(train_op, feed_dict={keep_prop_ph: 0.5})
